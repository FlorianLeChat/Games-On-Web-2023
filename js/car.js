...
{
	const itBOX = BABYLON.Mesh.CreateBox( "itBOX", 2, scene );
...
function createLights( scene )
{
	...
	const light0 = new BABYLON.DirectionalLight( "dir0", new BABYLON.Vector3( 0.5, -0.5, -0.5 ), scene );
...
{
	const camera = new BABYLON.FollowCamera( "CarFollowCamera", target.position, scene, target );
			...
			Car.move = () =>
			{
				...
				if ( inputStates.right )
				{
					if ( Car.laneIndex > 0 )
					{
						...
						const toPosition = new BABYLON.Vector3( lanes[ Car.laneIndex ], Car.position.y, Car.position.z );
				...
				if ( inputStates.left )
				{
					if ( Car.laneIndex < lanes.length - 1 )
					{
				...
				if ( inputStates.down )
				{
					if ( Car.laneIndex == 0 || Car.laneIndex == lanes.length - 1 )
